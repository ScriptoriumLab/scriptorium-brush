...
bool modian::tsf::dll::com_registration::register_profiles() {
	...
	hr = StringCchLength(infra::tsf::dll::info::MODIAN_IME_DESC.data(), STRSAFE_MAX_CCH, &len_of_desc);
	if (hr != S_OK) {
		goto Exit;
	}
	...
	hr = input_processor_profile_mgr->RegisterProfile(infra::tsf::dll::info::MODIAN_IME_CLSID, infra::tsf::dll::info::MODIAN_IME_LANG_ID, infra::tsf::dll::info::MODIAN_IME_GUID_PROFILE, infra::tsf::dll::info::MODIAN_IME_DESC.data(), static_cast<ULONG>(len_of_desc), nullptr, 0, static_cast<UINT>(-12), nullptr, 0, TRUE, 0);
...
void modian::tsf::dll::com_registration::unregister_profiles() {
	...
	input_processor_profile_mgr->UnregisterProfile(infra::tsf::dll::info::MODIAN_IME_CLSID, infra::tsf::dll::info::MODIAN_IME_LANG_ID, infra::tsf::dll::info::MODIAN_IME_GUID_PROFILE, 0);
...
bool modian::tsf::dll::com_registration::register_categories() {
	...
	for (const auto& guid : infra::tsf::dll::info::MODIAN_SUPPORT_CATEGORIES) {
		hr = category_mgr->RegisterCategory(infra::tsf::dll::info::MODIAN_IME_CLSID, guid, infra::tsf::dll::info::MODIAN_IME_CLSID);
	}
...
void modian::tsf::dll::com_registration::unregister_categories() {
	...
	for (const auto& guid : infra::tsf::dll::info::MODIAN_SUPPORT_CATEGORIES) {
		category_mgr->UnregisterCategory(infra::tsf::dll::info::MODIAN_IME_CLSID, guid, infra::tsf::dll::info::MODIAN_IME_CLSID);
	}
...
bool modian::tsf::dll::com_registration::register_server() {
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, ach_ime_key.c_str(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &reg_key_handle, &copied_string_len) == ERROR_SUCCESS) {
		if (RegSetValueEx(reg_key_handle, nullptr, 0, REG_SZ, reinterpret_cast<const BYTE*>(infra::tsf::dll::info::MODIAN_IME_DESC.data()), infra::tsf::dll::info::MODIAN_IME_DESC.size() * sizeof(WCHAR)) != ERROR_SUCCESS) {
			goto Exit;
		}

		if (RegCreateKeyEx(reg_key_handle, infra::tsf::dll::info::MODIAN_IME_REGINFO_KEY_INPROSVR32.data(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &reg_sub_key_handle, &copied_string_len) == ERROR_SUCCESS) {
			...
			if (RegSetValueEx(reg_sub_key_handle, infra::tsf::dll::info::MODIAN_IME_REGINFO_KEY_THREADMODEL.data(), 0, REG_SZ, reinterpret_cast<const BYTE*>(infra::tsf::dll::info::MODIAN_IME_MODEL.data()), infra::tsf::dll::info::MODIAN_IME_MODEL.size() * sizeof(WCHAR)) != ERROR_SUCCESS) {
				goto Exit;
			}
