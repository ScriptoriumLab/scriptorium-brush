...
TEST(concat_test, should_concat_wchart_raw_array_and_std_array_successfully) {
	...
	constexpr auto NEW_KEY_PATH = concat(ROOT_PATH, convert_clsid_to_wchar_t(CLSID_MODIAN_TEXT_SERVICE));

	ASSERT_EQ(NEW_KEY_PATH.size(), 66);
	ASSERT_STREQ(NEW_KEY_PATH.data(), L"SOFTWARE\\Microsoft\\CTF\\TIP\\{F7A3B6D1-EC88-41A2-9F5D-7A0E3C8A7B89}");
}