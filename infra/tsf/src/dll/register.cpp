...
bool modian::tsf::dll::com_registration::register_server() {
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, ach_ime_key.c_str(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &reg_key_handle, &copied_string_len) == ERROR_SUCCESS) {
		...
		if (RegCreateKeyEx(reg_key_handle, MODIAN_IME_REGINFO_KEY_INPROSVR32, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &reg_sub_key_handle, &copied_string_len) == ERROR_SUCCESS) {
			...
			if (RegSetValueEx(reg_sub_key_handle, MODIAN_IME_REGINFO_KEY_THREADMODEL, 0, REG_SZ, reinterpret_cast<const BYTE*>(MODIAN_IME_MODEL), (_countof(MODIAN_IME_MODEL)) * sizeof(WCHAR)) != ERROR_SUCCESS) {
				goto Exit;
			}
